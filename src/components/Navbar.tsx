  ...
  return (
    <header 
      className={`fixed w-full top-0 z-50 transition-all duration-300 ${
        isScrolled ? 'bg-white/90 dark:bg-black/90 backdrop-blur-md shadow-md py-2' : 'bg-transparent py-4'
      }`}
    >
        ...
        <nav className="hidden md:flex space-x-8 items-center">
          {['Home', 'Destinations', 'Tours', 'About', 'Contact'].map((item) => (
            <a 
              ...
              className="font-medium underline-animation text-foreground/90 hover:text-majestic-gold transition-colors"
            >
              {item}
            </a>
          ))}
